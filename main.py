...
def show_circle_shots(amount_of_shots, radius, half_side):
    ...
    np.savetxt("lab1_files/inside_circle.txt", inside_circle)
    np.savetxt("lab1_files/outside_circle.txt", outside_circle)


def exercise_1():
    # rozkład równomierny
    uniform_dist_numbers = uniform_distribution(-100, 100, 1000)
    np.savetxt("lab1_files/uniform_dist_numbers.txt", uniform_dist_numbers)

    # rozkład normalny
    normal_dist_numbers = normal_distribution(0,50,1000)
    np.savetxt("lab1_files/normal_dist_numbers.txt", normal_dist_numbers)
